...
#define UPDATE(win, rect) SDL_UpdateWindowSurfaceRects(win, rect, 1)
...
static SDL_Renderer* set_size(int w, int h, struct rect *r)
{
    SDL_Window *screen = SDL_CreateWindow("xwax-android", 
...
static int interface_main(void)
{
    ...
    renderer = set_size(width, height, &rworkspace);
    if (!renderer)
        return -1;
    ...
    for (;;) {
        ...
        case SDL_WINDOWEVENT:
            if(event.window.event == SDL_WINDOWEVENT_RESIZED) {
                renderer = set_size(event.window.data1, event.window.data2, &rworkspace);
                if (!renderer)
                    return -1;
        ...
        if (library_update) {
            UPDATE(renderer, &rlibrary);
            library_update = false;
        }

        if (status_update) {
            UPDATE(renderer, &rstatus);
            status_update = false;
        }

        if (decks_update) {
            UPDATE(renderer, &rplayers);
            decks_update = false;
        }
